...
if(isset($_POST["action"]))
{
 if($_POST["action"] == "fetch")
 {
  ...
  <table class="table table-bordered table-striped">
   <tr>
    <th>Nom Dosier</th>
    <th>Nombre Fichiers</th>
    <th>Taille</th>
    <th>Mettre à jour</th>
    <th>Supprimer</th>
    <th>Télécharger Fichier</th>
    <th>Voir les fichiers Téléchargés</th>
   </tr>
   ';
  if(count($folder) > 0)
  {
   foreach($folder as $name)
   {
    $output .= '
     <tr>
      ...
      <td><button type="button" name="update" data-name="'.$name.'" class="update btn btn-warning btn-xs">Mettre à jour</button></td>
      <td><button type="button" name="delete" data-name="'.$name.'" class="delete btn btn-danger btn-xs">Supprimer</button></td>
      <td><button type="button" name="upload" data-name="'.$name.'" class="upload btn btn-info btn-xs">Télécharger Fichier</button></td>
      <td><button type="button" name="view_files" data-name="'.$name.'" class="view_files btn btn-default btn-xs">Voir les fichiers</button></td>
     </tr>';
   }
  }
  else
  {
   $output .= '
    <tr>
     <td colspan="6">Pas encore de fichier</td>
    </tr>
   ';
  }
  $output .= '</table>';
 ...
 if($_POST["action"] == "create")
 {
  if(!file_exists($_POST["folder_name"])) 
  {
   ...
   echo 'Dossier crée avec succés';
  }
  else
  {
   echo 'Dossier déja crée';
  }
 }
 if($_POST["action"] == "change")
 {
  if(!file_exists($_POST["folder_name"]))
  {
   ...
   echo 'Nom du dossier a été bien modifié';
  }
  else
  {
   echo 'Dossier déja crée';
 ...
 if($_POST["action"] == "delete")
 {
  ...
  {
   echo 'Dossier Supprimé avec succés';
  }
 }
 ...
 if($_POST["action"] == "change_file_name")
 {
  ...
  {
   echo 'Fichier changé avec succés';
  }
  else
  {
